...
public class UsersController extends C {
    ...
    public String login() {
        ...
        if (userModel == null) {
            return this.ajaxMessage("准考证号或姓名错误", "信息");
        }
        ...
        List<ExamsModel> exams = ExamsModel.dao.find(
                "select * from exams where cardno=? and start_time<=? and end_time>?", cardno,
                nowTime, nowTime);
        ...
        for (int i = 0; i < exams.size(); i++) {
            ...
            if (userstatus == ExamlayoutUtil.USER_ONLINE
                    && layoutstatus != ExamlayoutUtil.LAYOUT_MOVE) {
                return this.ajaxMessage("当前帐号已登录，若有问题请联系管理员。", "已登录");
            }
            if (userstatus == ExamlayoutUtil.USER_ERROR
                    && layoutstatus != ExamlayoutUtil.LAYOUT_MOVE) {
        ...
        // 考试结束前提示（分）
        ecfg.setEnd_tips_time(TimeConvert.MinuteToSecond(Integer.parseInt(ConfigModel.dao
                .findByKey("examEndTipsTime"))));
        // 考试时长（分）
        int inter = TimeConvert.MinuteToSecond(Integer.parseInt(ConfigModel.dao
                .findByKey("examTime")));
        ecfg.setExam_time(inter < interval ? inter : interval);
        // 限制提交时间（分）
        ecfg.setLimit_submit(TimeConvert.MinuteToSecond(Integer.parseInt(ConfigModel.dao
                .findByKey("examSubmitTime"))));
        // 注意事项强制读取时间（秒）
