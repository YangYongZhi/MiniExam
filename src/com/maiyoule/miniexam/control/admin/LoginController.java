...
public class LoginController extends Controller {
	...
	public void dologin(){
		...
		if(chpwd==null){
			this.renderJson(Ajax.message("请输入正确的帐号和密码", "帐号"));
			return;
		}
		//验证密码
		...
		if(m.set("upassword", S.e(chpwd)).update()){
			this.renderJson(Ajax.message(this.getRequest().getAttribute(GUIConstants.BASEPATH) + "/contral", "成功",true));
		}else{
			this.renderJson(Ajax.message(this.getRequest().getAttribute(GUIConstants.BASEPATH) + "/contral", "成功",true));
		}
	...
	public void syncBaseData(){
		...
		//查询总数
		List<Integer> countobject=Db.use("db2").query(String.format("SELECT count(*) FROM %sBS_BANK",StringHelper.isNullOrEmpty(schame)?"":schame+"."));
		int count=0;
		...
		if(count>0){
			...
			for(int i=0;i<maxpage;i++){
				...
				String sql = String.format("SELECT * FROM (select ROW_NUMBER() OVER() AS ROWNUM,NO,NAME,PARENTNO,PARENTNAME,PCBNO,PCBNAME,CITYNO,CITYNAME,COUNTRYNO,COUNTRYNAME,ORGTYPENO,ORGTYPENAME,ISLEAF from %sBS_BANK) a where ROWNUM>%d and ROWNUM<=%d",StringHelper.isNullOrEmpty(schame)?"":schame+".",start,end);
				List<Object[]> items=Db.use("db2").query(sql);

				sqlbatch.clear();
				for(Object[] item:items){
					String tmpsql=String.format("insert into banks(no,name,parent_no,parent_name,pcb_no,pcb_name,city_no,city_name,country_no,country_name,orgtype_no,orgtype_name,isleaf) values('%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s')",
							toTrim(item[1]),toTrim(item[2]),toTrim(item[3]),toTrim(item[4]),toTrim(item[5]),toTrim(item[6]),toTrim(item[7]),toTrim(item[8]),toTrim(item[9]),toTrim(item[10]),toTrim(item[11]),toTrim(item[12]),toTrim(item[13]));
					sqlbatch.add(tmpsql);
				}
		...
		//查询总数
		List<Integer> areacount=Db.use("db2").query(String.format("SELECT count(*) FROM %sBS_AREA",StringHelper.isNullOrEmpty(schame)?"":schame+"."));
		count=0;
		...
		if(count>0){
			...
			for(int i=0;i<maxpage;i++){
				...
				int start = (int)(i * usersize);
				int end =(int)( (i+1) * usersize);
				String sql = String.format("SELECT * FROM (select ROW_NUMBER() OVER() AS ROWNUM,ID,NAME,PARENT,LEVEL,ISLEAF,SHORTNAME,STATUS,FLAG from %sBS_AREA) a where ROWNUM>%d and ROWNUM<=%d",StringHelper.isNullOrEmpty(schame)?"":schame+".",start,end);
				List<Object[]> items=Db.use("db2").query(sql);

				sqlbatch.clear();
				for(Object[] item:items){
					String tmpsql=String.format("insert into areas(id,name,parent,level,isleaf,short_name,status,flag) values(%s, '%s', %s, '%s', '%s', '%s', '%s', '%s')",
							toTrim(item[1]),toTrim(item[2]),toTrim(item[3]),toTrim(item[4]),toTrim(item[5]),toTrim(item[6]),toTrim(item[7]),toTrim(item[8]));
					sqlbatch.add(tmpsql);
				}
				Db.batch(sqlbatch, sqlbatch.size());
			}
		...
		for(Object[] item:orgtypes){
			sqlbatch.add(String.format("insert into orgtype(no,name,parent_no,parent_name) values('%s','%s','%s','%s')", toTrim(item[0]),toTrim(item[1]),toTrim(item[2]),toTrim(item[3])));
		}
		Db.batch(sqlbatch, sqlbatch.size());
		...
		for(Object[] item:pbcs){
			sqlbatch.add(String.format("insert into pcb(no,name,parent_no,parent_name,pcb_no,pcb_name) values('%s','%s','%s','%s','%s','%s')", toTrim(item[0]),toTrim(item[1]),toTrim(item[2]),toTrim(item[3]),toTrim(item[4]),toTrim(item[5])));
		}
		Db.batch(sqlbatch, sqlbatch.size());
