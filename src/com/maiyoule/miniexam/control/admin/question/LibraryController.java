...
public class LibraryController extends C {
	public void index() {
		...
		// 查询题库
		Page<LibraryModel> librarys = LibraryModel.dao.paginate(p, GUIConstants.PAGE_SIZE, "select *",
		        "from library order by id asc");
	...
	public String insert() {
		...
		// 检查是否存在
		LibraryModel lbl = LibraryModel.dao.findFirst(String.format("select * from library where name='%s'", libname));
		if (lbl != null) {
		...
		if (lbl.save()) {
			return this.ajaxMessage(this.getRequest().getAttribute(GUIConstants.BASEPATH) + "/contral/lib", "成功", true);
		} else {
	...
	public String del() {
		...
		/*
		 * I // 删除相关试题 // 删除回答 Db.update(
		 * "DELETE from a from answers a  INNER JOIN questions b ON a.question_id=b.id where b.libid='"
		 * + id + "'"); // 删除试题
		 * Db.update("DELETE from questions b where b.libid='" + id + "'"); //
		 * 删除题库 if (lib.delete()) { return this.ajaxMessage("删除成功", "成功", true);
		 * } else { return this.ajaxMessag("删除失败", "失败"); }
		 */
	}
