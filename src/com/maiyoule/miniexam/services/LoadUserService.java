...
public class LoadUserService {
    ...
    private void initBank() {
         ...
         */
        /* List<Object[]> db2bank = Db
                 .use("db2")
                 .query(String
                         .format("SELECT NO,NAME,PARENTNO,PARENTNAME,PCBNO,PCBNAME,CITYNO,CITYNAME,COUNTRYNO,COUNTRYNAME,ORGTYPENO,ORGTYPENAME,ISLEAF FROM %sBS_BANK",
                                 StringHelper.isNullOrEmpty(schame) ? "" : schame + "."));*/
        ...
        /* for (Object[] bank : db2bank) {
             ...
             sb.append(String.format(
                     "'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'", bank[0],
         ...
         */
        /* List<Object[]> db2pbc = Db
                 .use("db2")
                 .query(String
                         .format("SELECT NO,NAME,PARENTNO,PCBNO FROM %BS_PCB",
                                 StringHelper.isNullOrEmpty(schame) ? "" : schame + "."));
         List<String> insertPbcBatchSql = new ArrayList<String>();
         for (Object[] pbc : db2pbc) {
             sb.append("insert into banks(no,name,parent_no,parent_name,pcb_no,pcb_name,city_no,city_name,country_no,country_name,orgtype_no,orgtype_name,isleaf)");
             sb.append(" values(");
             sb.append(String.format(
                     "'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s'", pbc[0],
                     pbc[1], pbc[2], pbc[2], pbc[4], pbc[5], pbc[6], pbc[7], pbc[8],
                     pbc[9], pbc[10], pbc[11], pbc[12]));
             sb.append(");");
             insertPbcBatchSql.add(sb.toString());
             sb.setLength(0);
         }
    ...
    private void initUsers() {
        ...
        List<Object[]> db2allusers = Db
                .use("db2")
                .query(String
                        .format("SELECT ID,LOGINID,NAME,CARDTYPE,ORGCODEOFJR,ORGNAME,DEPTNAME,PHONE,USERSTATUS FROM %sBS_USERINFOOFJG ofjg WHERE ofjg.CARDTYPE !=''",
                                StringHelper.isNullOrEmpty(schame) ? "" : schame + "."));
        ...
        for (Object[] u : db2allusers) {
            ...
            sb.append(String.format("'%s','%s','%s','%s','%s','%s','%s',%d", cardno, name,
                    (cardtype.equals("身份证") ? "identity" : "passport"), bankno, bankName, phome,
                    department, (userStatus.equals("启用") ? 1 : 0)));
            sb.append(");");
            bathsql.add(sb.toString());
            sb.setLength(0);
            // 在DB2中查询该用户类型记录
            List<Object[]> db2usertype = Db
                    .use("db2")
                    .query(String
                            .format("SELECT ZXUSERTYPE,ZXUSERNAME,ORGCODE,ORGNAME FROM %sBS_USERINFOOFZX WHERE LOGINID='%s'",
